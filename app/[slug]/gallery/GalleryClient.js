...
export default function GalleryClient({ slug, settings }) {
  ...
  const loadFirstPage = useCallback(async () => {
    try {
      ...
      /* siet na akcii byva vrtkava - o 30 s to skusime znova */
    } finally {
      setLoading(false);
    }
  }, [slug]);
  ...
  async function loadMore() {
    ...
    try {
      const res = await fetch(
        `/api/e/${slug}/gallery?pageToken=${encodeURIComponent(nextToken)}`,
        { cache: 'no-store' }
      );
      if (!res.ok) return;
      const data = await res.json();
      ...
      <div className="layer">
        ...
        {nextToken && (
          <p style={{ textAlign: 'center', marginTop: 20 }}>
            <button type="button" className="btn btn--quiet btn--auto" onClick={loadMore}>
              {c.loadMore}
            </button>
          </p>
        )}
        ...
        <Foot lang={lang} onLang={setLang} settings={settings}>
          <p className="links" style={{ marginBottom: 18 }}>
            <Link href={`/${slug}`}>← {c.back}</Link>
            {settings.slideshowEnabled && (
              <>
                {' · '}
                <Link href={`/${slug}/slideshow`}>{c.slideshow}</Link>
              </>
            )}
          </p>
        </Foot>
